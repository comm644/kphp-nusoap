...
class nusoap_server extends nusoap_base
{
	...
	public bool $methodreturnisliteralxml = false;
	/**
	...
	public function __construct($wsdl = false)
	{
		...
		if ($wsdl) {
			...
			if (is_object($wsdl) && (get_class($wsdl) == 'wsdl')) {
				...
				$this->wsdl = new nusoap_wsdl($wsdl);
	...
	public function invoke_method()
	{
		...
		if ($this->wsdl) {
			...
			if ($this->opData) {
				...
				$this->appendDebug('opData=' . $this->varDump($this->opData));
				$this->methodname = (string)$this->opData['name'];
			} else {
		...
		} else if ((strlen($delim) > 0) && (substr_count($this->methodname, $delim) > 1)) {
		...
		if (!function_exists('call_user_func_array')) {
			...
			//FIXME: KPHP does not support eval()
			@eval($funcCall);
		} else {
			if ($class == '') {
				...
				//FIXME: KPHP does not support create class by name()
				$instance = new $class ();
	...
	public function serialize_return()
	{
		...
		// if fault
		if (isset($this->methodreturn) && is_object($this->methodreturn) && ((get_class($this->methodreturn) == 'soap\WSClient\NuSoap\soap_compat\soap_fault') || (get_class($this->methodreturn) == 'soap\WSClient\NuSoap\core\nusoap_fault'))) {
			...
			if ($this->wsdl) {
				...
				if (isset($this->methodreturn)) {
					$return_val = $this->serialize_val((string)$this->methodreturn, 'return');
				} else {
					$return_val = '';
	 ...
	 */
	public function configureWSDL(string $serviceName,
	                                     $namespace = false,
	                                     $endpoint = false,
	                              string $style = 'rpc',
	                                     ...
	                                     $schemaTargetNamespace = false)
	{
		...
		$schema = new nusoap_xmlschema('', '', $this->wsdl->namespaces);
		if ($style === 'document') {
			$schema->schemaInfo['elementFormDefault'] = 'qualified';
		}
		$schema->schemaTargetNamespace = $schemaTargetNamespace;
		$schema->imports['http://schemas.xmlsoap.org/soap/encoding/'][0] = array('location' => '', 'loaded' => true);
		$schema->imports['http://schemas.xmlsoap.org/wsdl/'][0] = array('location' => '', 'loaded' => true);
		$this->wsdl->schemas[$schemaTargetNamespace][0] = $schema;
